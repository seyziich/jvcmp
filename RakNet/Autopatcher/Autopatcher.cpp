...
{
	BitStream serializedFileDescriptor( ( const char* ) packet->data, packet->length, false );
	BitStream outputBitStream;
	DownloadableFileDescriptor *dfd;
	char *filePath;
	char SHA1Code[ SHA1_LENGTH ];
	bool allocatedFilePath;
	unsigned long numberOfDownloadableFiles;
	unsigned index;
	unsigned char packetID;
	assert( rakPeerInterface || rakClientInterface || rakServerInterface );
	assert( packet );
...
void AutoPatcher::OnAutopatcherRequestFiles( Packet *packet )
{
	...
	BitStream outputBitStream;
	DownloadableFileDescriptor dfd;
	unsigned long numberOfFilesRequested;
...
unsigned long AutoPatcher::GetDownloadStatus( char *filename, unsigned *fileLength, bool *fileDataIsCompressed, unsigned *compressedFileLength )
{
	if ( downloadingFiles.size() > 0 )
	{
		if ( filename )
