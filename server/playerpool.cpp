...
BOOL CPlayerPool::New(BYTE bytePlayerID, PCHAR szPlayerName)
{
	...
	if(m_pPlayers[bytePlayerID])
	{
		...
		pRcon->ConsolePrintf("%s has joined the game.",szPlayerName);
		logprintf("%s has joined the game.",szPlayerName);
...
BOOL CPlayerPool::Delete(BYTE bytePlayerID, BYTE byteReason)
{
	...
	pRcon->ConsolePrintf("%s has left the game. [%s]",m_szPlayerName[bytePlayerID],szQuitReasons[byteReason]);
	logprintf("%s has left the game. [%s]",m_szPlayerName[bytePlayerID],szQuitReasons[byteReason]);
	return TRUE;
}
