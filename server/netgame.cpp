...
extern BOOL		bDisableParkedCars;
				   ...
				   BYTE byteFriendlyFire, BYTE byteShowOnRadar)
{
	...
	if(!bDisableParkedCars)
		m_pVehiclePool->InitialiseFromConfig(DEFAULT_VEHICLES_FILE);
...
{	
	...
	if(!szParseMe) { 
		fatal_exit("PLAYERPOS setting missing!");
	}
	...
	if(!szParseMe) { 
		fatal_exit("CAMPOS setting missing!");
	}
	...
	if(!szParseMe) { 
		fatal_exit("CAMLOOK setting missing!");
	}
	...
	if(!szParseMe) { 
		fatal_exit("WORLDBOUNDS setting missing!");
	}
...
void CNetGame::AddBan(char * ip_mask)
{
	...
	FILE * fileBanList = fopen("banlist.lst","a");
...
{
	FILE * fileBanList = fopen("banlist.lst","r");
	if(!fileBanList) return;
