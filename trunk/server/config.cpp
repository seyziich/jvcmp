...
BOOL CConfig::LoadVehicleColours(char *szFile)
{
	TiXmlDocument* XMLdoc = new TiXmlDocument(szFile);
	if(!XMLdoc->LoadFile())
	{
		return FALSE;
	}

	int lTotalElements = 0;
	...
	while ( xmlColour )
	{
		...
		CCarCols* pCarCols = pCarColManager->Find( ucModel );
		if ( !pCarCols ) pCarCols = pCarColManager->New( ucModel );
