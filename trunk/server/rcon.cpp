...
void CRcon::OutputDeathMessage(int iDeathType,BYTE byteWhoDied,BYTE byteWhoKilled)
{
	...
	if(iDeathType == TEAM_KILL || iDeathType == VALID_KILL) {
		...
		logprintf("%s killed %s.",pPool->GetPlayerName(byteWhoKilled),pPool->GetPlayerName(byteWhoDied));
		...
		logprintf("%s died.",pPool->GetPlayerName(byteWhoDied));
